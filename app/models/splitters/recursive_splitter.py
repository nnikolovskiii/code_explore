...
from app.models.splitters.base_splitter import TextSplitter, Language
...
def _split_text_with_regex(
    ...
    matches = re.finditer(separator, text)
    split_indexes = [0]
    [split_indexes.append(match.start()) for match in matches]
    split_indexes.append(len(text))

    intervals = []
    for i in range(len(split_indexes) - 1):
        intervals.append((split_indexes[i], split_indexes[i + 1]))
    ...
    if separator:
        ...
        splits = list(text)
    return [s for s in splits if s != ""], intervals
...
class RecursiveCharacterTextSplitter(TextSplitter):
    ...
    def _split_text(self, text: str, separators: List[str]) -> List[Tuple[str, Tuple[int,int]]]:
        """Split incoming text and return chunks."""
        final_chunks = []
        ...
        splits, indexes = _split_text_with_regex(text, _separator, self._keep_separator)

        # Now go merging things, recursively splitting longer texts.
        _good_splits = []
        _separator = "" if self._keep_separator else separator
        for s, ind in zip(splits, indexes):
            if self._length_function(s) < self._chunk_size:
                _good_splits.append((s, ind))
            else:
                if _good_splits:
